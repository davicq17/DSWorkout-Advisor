	...
	const Registrar = async () => {
		...
		try {
			...
			if (existe == false) {
				if (permite == true) {
					await axios
						.post('http://127.0.0.1:8000/Usuarios/registro', {
							username: usernameI,
							name: nameI,
							surname: surnameI,
							email: emailI,
...
<div class="col-lg-7 container mt-5">
	<form class="row g-3">
		...
		<div
			id="specialityField"
			class="col-md-6 form-floating offset-md-6 mb-3"
			style:display={Speciality}
		>
			<input
				bind:value={especialidadI}
				type="text"
				class="form-control"
				id="inputSpeciality"
		...
		<div class="col-12">
			<button onclick={Registrar} type="submit" class="btn btn-primary"> Crear </button>
